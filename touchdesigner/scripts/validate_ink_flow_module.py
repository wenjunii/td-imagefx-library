...
def validate(write_report=True):
    ...
    saved = {
        ...
        "auto_time": ink_flow.par.Autotime.eval(),
        "manual_time": ink_flow.par.Manualtime.eval(),
        "visual_enabled": ink_flow.par.Visualenabled.eval(),
        "particles_enabled": ink_flow.par.Particlesenabled.eval(),
        "style": ink_flow.par.Style.eval(),
        "density": ink_flow.par.Particledensity.eval(),
        "seed": ink_flow.par.Seed.eval(),
    ...
    try:
        ...
        ink_flow.par.Seed = int(saved["seed"]) + 29
        seed_changed = _capture(ink_output)

        ink_flow.par.Seed = saved["seed"]
        ink_flow.par.Visualenabled = True
        ink_flow.par.Style = "ink_wash"
        ink_flow.par.Particlesenabled = True
        ...
        ink_flow.par.Autotime = saved["auto_time"]
        ink_flow.par.Manualtime = saved["manual_time"]
        ink_flow.par.Visualenabled = saved["visual_enabled"]
        ink_flow.par.Particlesenabled = saved["particles_enabled"]
        ink_flow.par.Style = saved["style"]
        ink_flow.par.Particledensity = saved["density"]
        ink_flow.par.Seed = saved["seed"]
